...
class _AsrScreenState extends State<AsrScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Center(child: Text("Asr Prayer")),
      ),
      body: ValueListenableBuilder<Box<NotesModel>>(
        valueListenable: Boxes.getData().listenable(),
        builder: (context, box, _) {
          var data = box.values.where((note) => note.title == 'Asr').toList();

          if (data.isEmpty) {
            return const Center(
              child: Text('No Asr prayers available.'),
            );
          }
  ...
  Future<void> _editDialog(NotesModel note) async {
    ...
    await showDialog(
      ...
      builder: (context) {
        ...
        return AlertDialog(
          title: const Text("Edit Asr Prayer Details"),
          content: Column(
    ...
    await showDialog(
      ...
      builder: (context) {
        return StatefulBuilder(
          builder: (context, setState) {
            return AlertDialog(
              title: const Text("Add Asr Prayer Details"),
              content: Column(
                mainAxisSize: MainAxisSize.min,
                children: [
                  DropdownButton(
