...
class _IshaScreenState extends State<IshaScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Center(child: Text("Isha Prayer")),
      ),
      body: ValueListenableBuilder<Box<NotesModel>>(
        valueListenable: Boxes.getData().listenable(),
        builder: (context, box, _) {
          var data = box.values.where((note) => note.title == 'Isha').toList();

          if (data.isEmpty) {
            return const Center(
              child: Text('No Isha prayers available.'),
            );
          }
  ...
  Future<void> _editDialog(NotesModel note) async {
    ...
    await showDialog(
      ...
      builder: (context) {
        ...
        return AlertDialog(
          title: const Text("Edit Isha Prayer Details"),
          content: Column(
    ...
    await showDialog(
      ...
      builder: (context) {
        return StatefulBuilder(
          builder: (context, setState) {
            return AlertDialog(
              title: const Text("Add Isha Prayer Details"),
              content: Column(
                mainAxisSize: MainAxisSize.min,
                children: [
                  DropdownButton(
                  ...
                  TextField(
                    ...
                    decoration: const InputDecoration(
                      labelText: 'Choose Time',
                      filled: true,
                      prefixIcon: Icon(Icons.timer_10_rounded),
                      enabledBorder:
                          OutlineInputBorder(borderSide: BorderSide.none),
                      focusedBorder: OutlineInputBorder(
                          borderSide: BorderSide(color: Colors.blue)),
